...
export default {
	...
	activate() {
		...
		this.workerFile = path.resolve(__dirname, "./worker.js");
	},
	...
	provideRangeCodeFormat() {
		return {
			formatCode: async (editor, range) => {
				...
				const results = await this.runWorker(fileDir, projectPath, text, config);
				return this.formatFixes(editor, range, text, results);
			},
			priority: 1,
	...
	formatFixes(editor, range, text, results) {
